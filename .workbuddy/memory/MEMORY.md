...
## 八、上线配置指南（2026-04-09 完成）

### 已生成文档
- `i妆小程序上线配置指南.md` - 完整上线配置步骤
- `i妆小程序上线配置清单.xlsx` - Excel版检查清单

### P0必配项（无此无法运行）
1. **微信支付申请** - 需要企业/个体户资质，对公银行账户
2. **云开发环境开通** - 微信开发者工具中开通
3. **数据库集合创建** - 8个集合（users/orders/tasks/courses/posts/teachers/vipRecords/coinRecords）

### P1重要配置（功能完整性）
4. API域名配置（当前使用云函数，暂不需要）
5. 课程内容上传 - 需要准备视频+封面图+课程数据

### P2上线前配置
6. 用户协议/隐私政策 - 需要替换真实内容
7. 客服与分享配置 - 微信公众平台配置
...
| 资质类 | 营业执照、法人身份证、对公账户 |
| 账号类 | 小程序AppID（已注册？） |
| 视觉类 | TabBar图标×8（81×81px PNG）、Logo、封面图 |
| 内容类 | 课程数据、老师信息 |
